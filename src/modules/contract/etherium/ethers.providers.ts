...
function validateBscNetwork(network: Networkish) {
  if (typeof network === 'number') {
    return [BINANCE_NETWORK.chainId, BINANCE_TESTNET_NETWORK.chainId].includes(
      network,
    );
  }

  if (typeof network === 'string') {
    return [BINANCE_NETWORK.name, BINANCE_TESTNET_NETWORK.name].includes(
      network,
    );
  }
...
): Promise<BaseProvider | AbstractProvider> {
  const {
  ...
  if (!useDefaultProvider) {
    ...
    if (cloudflare) {
      if (providerNetwork.chainId !== MAINNET_NETWORK.chainId) {
        throw new Error(
          `Invalid network. Cloudflare only supports ${MAINNET_NETWORK.name}.`,
        );
      }
    ...
    if (custom) {
      const customInfos: (ConnectionInfo | string)[] = !Array.isArray(custom)
        ? [custom]
        : custom;
  ...
  return {
    ...
    useFactory: async (provider: AbstractProvider): Promise<EthersContract> => {
      return await lastValueFrom(
        defer(async () => new EthersContract(provider)),
      );
    },
